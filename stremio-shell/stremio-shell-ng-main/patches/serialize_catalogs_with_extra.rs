...
use stremio_core::models::catalogs_with_extra::{Catalog, CatalogPage};
use stremio_core::types::resource::MetaItemPreview as CoreMetaItemPreview;
...
/// Merge all loaded catalog pages for the WebUI board/discover preview.
///
/// Upstream only exposes `catalog.first()` and `.take(10)`, which makes
/// `CatalogsWithExtra::LoadNextPage` invisible to MetaRow. MyStremio flattens
/// every Ready page (deduped by id) so lazy row loading can grow past 10.
fn serialize_catalog_content<'a>(
    pages: &'a Catalog<CoreMetaItemPreview>,
    first: &'a CatalogPage<CoreMetaItemPreview>,
    ctx: &'a Ctx,
    streaming_server: &stremio_core::models::streaming_server::StreamingServer,
) -> Option<Loadable<Vec<MetaItemPreview<'a>>, String>> {
    let mut ready_pairs: Vec<(
        &'a stremio_core::types::addon::ResourceRequest,
        &'a CoreMetaItemPreview,
    )> = Vec::new();
        ...
        match &page.content {
            Some(Loadable::Ready(meta_items)) => {
                for meta_item in meta_items {
                    ready_pairs.push((&page.request, meta_item));
                }
            }
            Some(Loadable::Loading) => {
                saw_loading = true;
            }
            Some(Loadable::Err(_)) | None => {}
        }
    }

    if !ready_pairs.is_empty() {
        let poster_shape = ready_pairs
            .first()
            .map(|(_, meta_item)| &meta_item.poster_shape);
        return Some(Loadable::Ready(
            ready_pairs
                .into_iter()
                .unique_by(|(_, meta_item)| &meta_item.id)
                .map(|(request, meta_item)| MetaItemPreview {
                    meta_item,
                    ...
                    in_library: ctx
                        ...
                        .unwrap_or_default(),
                    deep_links: MetaItemDeepLinks::from((meta_item, request)).into_web_deep_links(),
                })
                .collect::<Vec<_>>(),
        ));
    }
    ...
    if saw_loading {
        return Some(Loadable::Loading);
    }

    match &first.content {
        Some(Loadable::Ready(_)) => Some(Loadable::Ready(Vec::new())),
            ...
            catalogs: catalogs_with_extra
                ...
                .filter_map(|pages| {
                    ...
                    ctx.profile
                        .addons
                        .iter()
                        .find(|addon| addon.transport_url == first.request.base)
                        .and_then(|addon| {
                            addon
                                .manifest
                                .catalogs
                                ...
                                .find(|manifest_catalog| {
                                    manifest_catalog.id == first.request.path.id
                                        && manifest_catalog.r#type == first.request.path.r#type
                                })
                                .map(|manifest_catalog| (addon, manifest_catalog, pages, first))
                        })
                })
                .map(|(addon, manifest_catalog, pages, first)| ResourceLoadable {
                    ...
                    content: serialize_catalog_content(pages, first, ctx, streaming_server),
                    deep_links: DiscoverDeepLinks::from(&first.request).into_web_deep_links(),
                })
                .collect::<Vec<_>>(),
        }
    }
