(function () {
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      #${ROOT_ID} .mystremio-am-list {
        ...
        overflow-y: auto;
        box-sizing: border-box;
        padding: 0.4rem 1.25rem 1.5rem 0;
      }
      #${ROOT_ID} .mystremio-am-card {
        position: relative;
        display: block;
      ...
      #${ROOT_ID} .mystremio-am-icon.mystremio-am-configure:hover {
        outline: var(--focus-outline-size, 0.15rem) solid var(--secondary-accent-color);
        outline-offset: 0;
        background-color: transparent;
      }
      #${ROOT_ID} .mystremio-am-share {
        flex: none;
        display: inline-flex;
        align-items: center;
        justify-content: center;
      ...
      #${ROOT_ID} .mystremio-am-share:hover .mystremio-am-share-label {
        text-decoration: underline;
      }
      #${ROOT_ID} .mystremio-am-share svg {
        flex: none;
        width: 1.5rem;
        height: 1.5rem;
      }
      #${ROOT_ID} .mystremio-am-share-label {
        font-size: 1.1rem;
        font-weight: 700;
      }
      #${ROOT_ID} .mystremio-addon-soft-toggle {
  ...
  function renderCard(addon, index) {
    ...
    if (isConfigurable(addon)) {
      const configure = iconButton(t('Configure', 'Konfigurieren'), ICON_SETTINGS);
    ...
    const shareUrl = addonTransportUrl(addon);
    if (shareUrl) {
      const share = document.createElement('button');
      share.type = 'button';
      share.className = 'mystremio-am-share';
      share.title = t('Share', 'Teilen');
      share.setAttribute('aria-label', t('Share', 'Teilen'));
      share.innerHTML = `${ICON_SHARE}<span class="mystremio-am-share-label">${t('Share', 'Teilen')}</span>`;
      share.addEventListener('click', () => {
        const url = addonTransportUrl(addon);
        if (!url) return;
        const done = () => showToast(t('Addon URL copied.', 'Addon-URL kopiert.'));
        if (navigator.clipboard?.writeText) {
          navigator.clipboard.writeText(url).then(done).catch(() => {
            window.prompt(t('Copy addon URL', 'Addon-URL kopieren'), url);
          });
        } else {
          window.prompt(t('Copy addon URL', 'Addon-URL kopieren'), url);
        }
