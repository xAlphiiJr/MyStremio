...
3. Create a GitHub Release with tag `v{version}` (example: `v2.3.3`)
...
1. The release tag version must be **higher** than the installed app version (semver, e.g. `2.3.2` → `2.3.3`).
2. Both assets must be attached to that release (installer + `SHA256SUMS.txt`).
3. Names must match: `MyStremioSetup-v2.3.3_x64.exe` and a `SHA256SUMS.txt` line for that exact filename.
